...
export function appendThinking(blocks: StreamBlock[], delta: string): StreamBlock[] {
  ...
  // GLM/NeuralWatt may interleave reasoning tokens after early content; keep one Thinking block.
  if (last?.type === "text") {
    const firstThinking = next.findIndex((b) => b.type === "thinking")
    if (firstThinking >= 0) {
      const block = next[firstThinking]
      if (block.type === "thinking") {
        next[firstThinking] = { type: "thinking", text: block.text + delta }
        return next
      }
    }
  }
